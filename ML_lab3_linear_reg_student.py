...
def calcRMSE(labelsAndPreds):
    ...
    """
    m = labelsAndPreds.map(lambda (a,b): squaredError(a,b)).reduce(lambda a,b: a+b) / labelsAndPreds.count()
    return m**0.5
...
# TODO: Replace <FILL IN> with appropriate code
labelsAndPreds = parsedValData.map(lambda x: (firstModel.predict(x.features), x.label))
...
def twoWayInteractions(lp):
    ...
    """
    m = list(x*y for (x,y) in list(itertools.product(lp.features, repeat=2)))
    ret = LabeledPoint(lp.label, np.hstack((lp.features, m)))
    return ret
