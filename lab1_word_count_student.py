...
wordCountsCollected = (wordsRDD
                       .map(lambda x: (x, 1)).groupByKey().map(lambda (k,v): (k,sum(v))).reduceByKey(lambda a,b: a+b)
                       .collect())
...
# TODO: Replace <FILL IN> with appropriate code
uniqueWords = wordCounts.map(lambda (k,v): 1).reduce(lambda a,b: a+b)
print uniqueWords
...
def wordCount(wordListRDD):
    ...
    """
    return wordListRDD.map(lambda x: (x, 1)).groupByKey().map(lambda (k,v): (k,sum(v))).reduceByKey(lambda a,b: a+b)
