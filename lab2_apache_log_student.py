...
uniqueHostCount = uniqueHosts.map(lambda s: 1).reduce(lambda a, b: a+b)
...
dayHostCount = dayGroupedHosts.map(lambda (u,v): (u[0],1))

dailyHosts = (dayHostCount
              .reduceByKey(lambda a, b: a+b).sortByKey().cache())
...
sortedByDay = groupedByDay.map(lambda (u,v): (u[0],(v,1)))

avgDailyReqPerHost = (sortedByDay
                      .reduceByKey(lambda a, b: (a[0]+b[0], a[1]+b[1])).map(lambda (u,v): (u,v[0]/v[1])).sortByKey().cache())
...
badEndpointsTop20 = badEndpointsSum.takeOrdered(20, lambda s: s[1] * -1)
...
errHostsTop25 = errHostsSum.takeOrdered(25, lambda s: s[1] * -1)
...
errDateCountPairTuple = badRecords.map(lambda log: ((log.date_time.day,log.host), 1 ))

errDateSum = errDateCountPairTuple.reduceByKey(lambda a, b: a + b).map(lambda (u,v): (u[0],v))
...
hourCountPairTuple = badRecords.map(lambda log: ((log.date_time.hour,log.host), 1 ))

hourRecordsSum = hourCountPairTuple.reduceByKey(lambda a, b: a + b).map(lambda (u,v): (u[0],v))
