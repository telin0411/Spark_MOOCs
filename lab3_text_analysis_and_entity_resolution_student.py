...
def findBiggestRecord(vendorRDD):
    ...
    """
    return vendorRDD.takeOrdered(1, lambda s: len(s[1])*-1)
...
def idfs(corpus):
    ...
    """
    N = corpus.map(lambda (k,v): 1).reduce(lambda a,b: a+b)
    uniqueTokens = corpus.flatMap(lambda (k,v): [(x,1) for x in set(v)])
    tokenCountPairTuple = uniqueTokens.reduceByKey(lambda a,b: a+b)
    tokenSumPairTuple = tokenCountPairTuple.map(lambda (k,v): (k,float(N)/float(v)))
...
def tfidf(tokens, idfs):
    ...
    """
    tfs = tf(tokens)
    tfIdfDict = {x: tfs[x] * idfs[x]  for x in tfs}
    return tfIdfDict
...
def dotprod(a, b):
    ...
    """
    return sum({x: a[x] * b[x] if x in b else a[x] * 0 for x in a}.values())
...
def norm(a):
    ...
    """
    return sum({x: a[x]**2 for x in a}.values())**0.5
...
avgSimDups = trueDupsRDD.map(lambda (k,v): v[0]).filter(lambda s: s!='gold').reduce(lambda a,b: a+b) / float(trueDupsCount)
...
commonTokens = (amazonInvPairsRDD
                .join(googleInvPairsRDD).map(swap).reduceByKey(lambda a,b: a+b)
                .cache())
