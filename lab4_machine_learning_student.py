...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    """
    ret = (len(IDandRatingsTuple[1]), float(sum(IDandRatingsTuple[1])) / len(IDandRatingsTuple[1]))
    return (IDandRatingsTuple[0], ret)
...
movieNameWithAvgRatingsRDD = (moviesRDD
                              .join(movieIDsWithAvgRatingsRDD).map(lambda (a,b): (b[1][1], b[0], b[1][0])))
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        .join(actualReformattedRDD).map(lambda (a,b): (b[0]-b[1])**2))
...
trainingAvgRating = trainingRDD.map(lambda (a,b,c): c).reduce(lambda a,b: a+b) / trainingRDD.count()
...
myUnratedMoviesRDD = (moviesRDD
                      .subtractByKey(myRatingsRDD.map(lambda (a,b,c): (b,c))).map(lambda (a,b): (0,a)))
...
movieCountsRDD = movieIDsWithAvgRatingsRDD.map(lambda (a,b): (a, b[0]))
